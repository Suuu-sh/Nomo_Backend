...
- Admin is not a DB role and not a seeded special table row. Backend admin endpoints check the authenticated Supabase user email against `TOMO_ADMIN_EMAILS` in Render env.
...
- `TOMO_ADMIN_EMAILS` の Render env 設定漏れがあると admin UI が使えない。
...
3. Confirm sign-in through `scripts/tomo_supabase_runtime_check.py`.
4. Confirm Backend through `scripts/tomo_backend_smoke.py`.
