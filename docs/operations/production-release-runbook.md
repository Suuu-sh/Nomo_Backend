# Tomo production release runbook

Last updated: 2026-05-28

This runbook defines when to move `development` into `main` and what must be checked together for Backend, Supabase, and TestFlight. Tomo is pre-release, so normal implementation work still lands on `development`; production reflection happens as one coordinated release window.
