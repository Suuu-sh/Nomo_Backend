...
この文書は、Tomo Backend を今後 **AI駆動開発で安全に変更しやすい構造** に寄せるための実装ガイドである。

採用判断の概要は `docs/adr/0001-ai-driven-feature-slice-clean-architecture.md` を参照する。

## 結論

Tomo Backend は今後、**Feature Slice 型の軽量 Clean Architecture** を基本方針にする。
...
後者は理論上きれいだが、Tomo の規模と AI 駆動開発では変更箇所が遠くなりやすい。
...
Tomo Backend は Supabase RLS を重要な安全境界として使う。
...
今後の Tomo Backend では、以下を標準判断にする。
...
memory 削除時、返却 row に `photo_path` がある場合は Supabase Storage の `tomo-photos` object 削除を試みる。
...
- `/tomo-notification-worker` binary
- Render cron は本番未使用
...
- Tomo の Supabase migrations は Mobile repository 側で管理され、GitHub Actions が dev / production に順次適用する。
