...
func (r *router) health(w http.ResponseWriter, _ *http.Request) {
	writeJSON(w, http.StatusOK, map[string]string{"status": "ok", "service": "tomo-backend"})
}
...
func (r *router) createMemory(w http.ResponseWriter, req *http.Request, authToken string) {
	...
	row, err := r.memoryUsecase(req).CreateMemory(req.Context(), memories.CreateInput{
		AuthToken:             authToken,
		OwnerUserID:           req.Header.Get("X-Tomo-User-ID"),
...
func (r *router) auth(next func(http.ResponseWriter, *http.Request, string)) http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		cleanUserID, errMessage := cleanUUID(userID, "X-Tomo-User-ID")
		if errMessage != "" {
		...
		req.Header.Set("X-Tomo-User-ID", authUserID)
		next(w, req, token)
	}
}
...
func (r *router) withCORS(next http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		w.Header().Set("Access-Control-Allow-Headers", "Authorization, Content-Type, X-Tomo-User-ID")
