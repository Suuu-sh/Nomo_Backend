...
func (r *router) listDrinkLogs(w http.ResponseWriter, req *http.Request, authToken string) {
	...
	selectColumns := "id,owner_user_id,drank_at,place_name,memo,photo_path,link_url,is_official,owner:profiles!drink_logs_owner_user_id_fkey(id,user_id,display_name,character_key,avatar_url,is_plus),drink_log_likes(user_id,profiles(id,user_id,display_name,character_key,avatar_url,is_plus)),drink_log_friends(profiles(id,user_id,display_name,character_key,avatar_url,is_plus))"
	q := url.Values{}
