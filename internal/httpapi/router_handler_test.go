...
func authedRequest(method, path, body string) *http.Request {
	...
	req.Header.Set("X-Tomo-User-ID", testUserID)
	return req
}
...
func TestAuthRejectsUserIDMismatch(t *testing.T) {
	...
	req.Header.Set("X-Tomo-User-ID", otherUserID)
...
func TestCreateMediaUploadURLCreatesUserScopedMemoryPhotoPath(t *testing.T) {
	fake := newFakeSupabase(t, func(w http.ResponseWriter, req *http.Request) {
		if strings.HasPrefix(req.URL.Path, "/storage/v1/object/upload/sign/tomo-photos/users/"+testUserID+"/memories/") {
			writeFakeJSON(w, http.StatusOK, map[string]any{
				"url":   "/object/upload/sign/tomo-photos/users/" + testUserID + "/memories/photo.jpg?token=upload-token",
				"token": "upload-token",
			})
			return
		}
	...
	if body["bucket"] != "tomo-photos" || body["token"] != "upload-token" || body["content_type"] != "image/jpeg" {
	...
	request, ok := fake.lastRequest("/storage/v1/object/upload/sign/tomo-photos/" + path)
	if !ok {
