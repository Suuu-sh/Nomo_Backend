...
func TestCreateDisplayURLValidatesMemoryPhotoPath(t *testing.T) {
	...
	if result.SignedURL != "https://example.test/display" || storage.displayPath != "tomo-photos/users/"+testUserID+"/memories/photo.jpg" {
...
func TestEscapedStoragePathKeepsFolderSeparators(t *testing.T) {
	got := escapedStoragePath("tomo-photos", "users/abc/memories/photo 1.jpg")
	if !strings.Contains(got, "/") || got != "tomo-photos/users/abc/memories/photo%201.jpg" {
